...
def main():
    ...
    data = {"meta": meta, "S": _STR, "items": out_items, "checks": checks,
        ...
        "moves": moves, "nm": name_of, "mix_notes": M["mix_notes"],
            ...
            # ШТУКИ → ТОННЫ ПО КОЭФФИЦИЕНТУ 1С — ТОЛЬКО ДЛЯ ВКЛАДКИ «ОСТАТКИ»
            # (заказчик 16.09.2026: «можем сделать это только для вкладки
            # остатки, а в остальном это правило не применялось»). Код → сколько
            # штук в тонне, если в справочнике «единица для отчётов = т»,
            # базовая — штучная и коэффициент НЕ равен 1 (заглушки «1 шт = 1 т»
            # у электродвигателя и ТМ-6,3 не берём; колёсная пара 0,834 шт/т —
            # настоящий вес 1,2 т). Везде, кроме «Остатков»,
            # правило §7 в силе: штуки в тонны не превращаются.
            "nm_coef": {c_: round(float(_i["coef"]), 4) for c_, n_ in name_of.items()
                        for _i in [nom_by_name.get(n_) or {}]
                        if _i.get("unit_report") == "т"
                        and _i.get("unit_base") in C.UNIT_NON_WEIGHT
                        and (_i.get("coef") or 0) > 0 and abs(_i["coef"] - 1.0) > 1e-9},
