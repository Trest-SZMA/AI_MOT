...
check("экономика: независимая оценка по факту", 'id="outcome"' in e and "Ожидаемый результат" in e and "Точность модели" in e)
check("шапка: площадка по адресам лота", "по адресам лота" in h or "по регистру затрат" in h)
